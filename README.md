...
3. The first launch opens a short setup window that asks for everything in one place, each with its reason: **Microphone** (so your own voice can be transcribed), **Speech Recognition**, **system audio**, **Screen Recording** (only needed on Macs where the Core Audio Process Tap can't capture system audio — no video is recorded) and **Keychain** access for your API key. Nothing else in the app raises a macOS permission dialog. If you skip something, the overlay shows an **Open Setup** button when it matters.
...
The same goes for the **Keychain**: your saved API key is tied to the build's signature too, so after an update macOS asks once to let Whisper Pilot open it again (choose **Always Allow**). Older versions saved keys as two separate Keychain items (`gemini.api_key`, `anthropic.api_key`, service `com.whisperpilot.app`); they are copied into one item on the first unlock and left in place. Once everything works you can delete those two in Keychain Access.
...
- **`~/Library/Application Support/com.whisperpilot.app/runtime.log`** — Whisper Pilot's own rolling log, written line-by-line. The tail is what was happening immediately before the crash. The app surfaces the last ~20 lines automatically on the next launch via the in-app log pane. The log never contains what was said in a meeting — only counts, timings and errors — and it trims itself to the newest 256 KB once it passes 1 MB.
